...
export function IRBCharterModal({ open, onOpenChange }: IRBCharterModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto bg-slate-950 text-slate-100 border-slate-800">
        <DialogHeader>
        ...
        {loading ? (
          <div className="p-8 text-center text-slate-400">Loading IRB Governance Charter...</div>
        ) : charter ? (
          <div className="space-y-4 text-xs mt-2">
            <div className="p-3.5 rounded-lg bg-slate-900 border border-slate-800 flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="font-semibold text-white text-sm">{charter.title}</p>
                <div className="flex items-center gap-3 mt-1 text-slate-400 text-[11px]">
                  <span>IRB Approval: <strong className="text-amber-400 font-mono">{charter.irbApprovalNumber}</strong></span>
                  <span>•</span>
                  <span>Version: <strong className="text-emerald-400 font-mono">{charter.version}</strong></span>
                  <span>•</span>
                  <span>Effective: <strong className="text-slate-200">{charter.effectiveDate}</strong></span>
                </div>
              </div>
            ...
            <div className="space-y-3">
              {charter.sections.map((sec, idx) => (
                <div key={idx} className="p-4 rounded-lg bg-slate-900/80 border border-slate-800 space-y-1.5">
                  <h4 className="font-bold text-white text-sm flex items-center gap-2">
                    <FileText className="w-4 h-4 text-rose-400" />
                    {sec.heading}
                  </h4>
                  <p className="text-slate-300 text-xs leading-relaxed">{sec.content}</p>
                </div>
              ))}
            </div>

            <div className="p-3.5 rounded-lg bg-rose-950/30 border border-rose-800/50 text-rose-200 flex items-start gap-2.5">
