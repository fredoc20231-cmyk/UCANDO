        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4 shadow-xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950 text-emerald-600 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/40 shadow-md">
                <Lock className="w-6 h-6" />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-xl font-bold text-slate-900 dark:text-white">Immutable WORM Audit & Compliance Stream</h1>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 text-slate-700 dark:text-slate-300">
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/60">
                    <td className="py-3 px-3 text-emerald-600 dark:text-emerald-400 font-bold">{log.id}</td>
                    <td className="py-3 px-3 text-slate-500 dark:text-slate-400">{new Date(log.timestamp).toLocaleTimeString()}</td>
                    <td className="py-3 px-3">
                      <span className="text-slate-900 dark:text-white block font-bold">{log.actor}</span>
                      <span className="text-[10px] text-slate-500">{log.actorRole}</span>
                    </td>
                    <td className="py-3 px-3">
                      <Badge variant="outline" className="border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-[10px] mb-1">
                        {log.action}
                      </Badge>
                      <p className="text-slate-700 dark:text-slate-300 text-[11px]">{log.resource}</p>
                    </td>
                    <td className="py-3 px-3">
                      <Badge
                        className={
                    ...
                    <td className="py-3 px-3 text-right">
                      <button
                        ...
                        className="inline-flex items-center text-[10px] text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
                      >
                        <span className="truncate max-w-[120px] inline-block mr-1">{log.sha256Hash}</span>
                        {copiedId === log.id ? <Check className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3 h-3" />}
                      </button>
                    </td>
                  </tr>
                ))}
